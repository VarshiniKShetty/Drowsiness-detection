...
def writeEyes(a, b, img):
    ...
    x1 = a[0][0]
    x2 = a[3][0]
    print(f"Left eye coordinates: ({x1}, {y1}) to ({x2}, {y2})")
    if y2 > y1 and x2 > x1:
        cv2.imwrite('left-eye.jpg', img[y1:y2, x1:x2])
    else:
        print("Invalid left eye coordinates, skipping save.")

    y1 = max(b[1][1], b[2][1])
    y2 = min(b[4][1], b[5][1])
    x1 = b[0][0]
    x2 = b[3][0]
    print(f"Right eye coordinates: ({x1}, {y1}) to ({x2}, {y2})")
    if y2 > y1 and x2 > x1:
        cv2.imwrite('right-eye.jpg', img[y1:y2, x1:x2])
    else:
        print("Invalid right eye coordinates, skipping save.")
